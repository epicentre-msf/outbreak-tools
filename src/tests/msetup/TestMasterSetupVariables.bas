...
Public Sub TestInitialize()
    ...
    DeleteWorksheets VARIABLES_SHEET, CHOICES_SHEET, DROPDOWNS_SHEET, SOURCE_SHEET, TARGET_SHEET
    Set FixtureSheet = EnsureWorksheet(VARIABLES_SHEET)
    ...
    lo.HeaderRowRange.Cells(1, 4).Value = "Label"
...
Public Sub TestCreateEnsuresAllColumns()
    ...
    Assert.AreEqual "Label", lo.ListColumns(4).Name, "Column name 'Variable Label' should be present"
...
Public Sub TestImportFromWorksheetHandlesOffsetHeaders()
    ...
    headers = Array("Variable Order", "Variable Section", "Variable Name", "Label", _
    ...
    Set sourceSheet = EnsureWorksheet(SOURCE_SHEET)
...
Public Sub TestCopyToListObjectAppliesNameStyleAndOrder()
    ...
    Set targetSheet = EnsureWorksheet(TARGET_SHEET)
...
Private Function BuildChoicesStub() As LLChoices
    ...
    Set sh = EnsureWorksheet(CHOICES_SHEET)
    sh.Cells.Clear
...
Private Function BuildDropdownsStub() As DropdownLists
    ...
    Set sh = EnsureWorksheet(DROPDOWNS_SHEET)
    sh.Cells.Clear
...
Private Sub AssertColumnOrder(ByVal lo As ListObject)
    ...
    expected = Array("Variable Order", "Variable Section", "Variable Name", "Label", _
                     "Default Choice", "Choices Values", "Default Status", "Comments")
