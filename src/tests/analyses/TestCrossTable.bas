...
'@TestMethod("CrossTable")
'@sub-title Every cell a GLOBAL SUMMARY table writes sits inside a named range.
'@details
'The coverage gate for Session 125 on the scope `NameRanges` used to leave
'unnamed. When this test was first written it pinned the gap instead: the
'Filtered data header sat outside every name, because COLGS_SET names the
'All data cell alone. COLGS_FILTERED covers it now. The two value cells of a
'global summary row are not exercised here, because the table builder does not
'write them -- TestCrossTableFormula covers those with the formulas in place.
Public Sub TestEveryWrittenCellOfAGlobalSummaryTableIsNamed()
    CustomTestSetTitles Assert, "CrossTable", "TestEveryWrittenCellOfAGlobalSummaryTableIsNamed"
    On Error GoTo TestFail
    ...
    Assert.AreEqual vbNullString, uncovered, _
                    "A global summary table leaves no written cell outside its names"

    Exit Sub
TestFail:
    CustomTestLogFailure Assert, "TestEveryWrittenCellOfAGlobalSummaryTableIsNamed", Err.Number, Err.Description
End Sub
