...
Sub BuildList(D_TitleDic As Scripting.Dictionary, T_dataDic, D_Choices As Scripting.Dictionary, T_Choices, T_Export)
    ...
    Dim iDecNb As Integer                        'pour le calcul de d�cimal
    Dim k As Integer
    ...
    With xlsApp
        ...
        .Workbooks.Add
        .ActiveWorkbook.VBProject.References.AddFromFile ("C:\windows\system32\scrrun.dll") 'on coche le scripting runtime pour acceder au dico / remplac� par CLA_dictionary
        ...
        'd�placement des cl�s de sauvegarde
        Call TransfertSheet(xlsApp, "PASSWORD")
    
        'd�placement de la Translation
        ...
        'premier tour pour la cr�ation des feuilles
        i = 1
        j = 0
        ReDim T_NbCol(j)
        sPrevSheetName = ""
...
Private Sub TransfertSheet(xlsApp As Object, sSheetName As String)
    ...
    ActiveWorkbook.SaveAs "C:\LineListeApp\tampon.xlsx" 'puisqu'on ne peut pas balancer une feuille vers une autre instance, on cr�e un fichier tampon
...
Private Function LetWordingWithSpace(xlsApp As Excel.Application, sDataWording As String, sSheetName As String)
    'on colle un espace apr�s des libell� en doublon pour �viter que excel les incr�mente
