...
' @description Update formulas in a single spatial table when user changes admin level.
' @param rngName Named range of the admin dropdown (e.g. "ADM_DROPDOWN_sp1")
Public Sub UpdateSingleSpTable(ByVal rngName As String)
    ...
    Set geoObj = LLGeo.Create(ThisWorkbook.Worksheets(GEOSHEET))
    adminName = geoObj.AdminCode(selectedAdmin)
...
'@section Divide by Population
'===============================================================================

' @description Divide or revert all computed values by the population.
' @param rngName Named range of the population factor (e.g. "POPFACT_sp1")
' @param revertBack True to remove the division and restore original formulas
...
' @description Toggle population division formatting.
' @param rngName Named range of the divide/no-divide dropdown
