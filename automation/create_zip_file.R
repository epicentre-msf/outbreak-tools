...
actual_branch <- dplyr::case_when(
  actual_branch == "dev" ~ "dev",
  actual_branch == "hot-fixes" ~ "dev",
  actual_branch == "main" ~ "main"
)
...
file.copy(
  from = glue::glue("{obt_folder}/src/bin/designer_{actual_branch}.xlsb"),
  to = glue::glue(
    "{obt_all_folder}/",
...
utils::zip(
  ...
  files = c(
    ...
    glue::glue(
      "{obt_all_folder}/_ribbontemplate_{actual_branch}-{rel_date}.xlsb"
    ),
