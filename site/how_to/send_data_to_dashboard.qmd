...
OBT linelists developed by the EpiDS team are usually paired with a [dashboard](../how_to/connect_to_dashboard.qmd), at least in the case of intersection linelists or big multi-sections projects. As a consequence, all linelists created by the EpiDS team can generate *password-protected anonymous data*, ready to be sent to Epicentre for compilation and visualisation.

If you are using a linelist for which a dashboard was developed, you can send data to Epicentre. The scripts run automatically every hour, so you can choose the frequency at which you send the data depending on your needs.
...
**The name of the export file is generated automatically based on the metadata you filled** so make sure these data are filled correctly once and for all. 


::: {.callout-tip}
If the export names are correct, you can send several exports in the same mail (from different sites, or even different diseases), the scripts will parse the export names to sort the files on our end.
:::
